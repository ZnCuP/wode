 ...
 * 获取当前语言下的字段值
 */
function getLocalizedField(product, key) {
    const baseValue = product[key];
    const localizedValue = product[`${key}_zh`];
    if (productsCurrentLanguage === 'zh') {
        return localizedValue || baseValue || '';
    }
    return baseValue || localizedValue || '';
}

/**
 * 获取当前语言下的文本数组
 */
function getLocalizedList(items) {
...
function renderProductCard(product) {
    ...
    const productSubtitle = getLocalizedField(product, 'subtitle') || getLocalizedField(product, 'description');
    const highlightSource = (Array.isArray(product.card_highlights) && product.card_highlights.length)
        ? product.card_highlights
        : (Array.isArray(product.features)
            ? product.features.map(feature => ({ en: feature.title, zh: feature.title_zh }))
            : []);
    const highlights = getLocalizedList(highlightSource).slice(0, 3);
    const highlightsHTML = highlights.length
        ? `<ul class="product-card__highlights">${highlights.map(item => `<li>${item}</li>`).join('')}</ul>`
        : '';
    ...
    return `
        <div class="product-card" data-product-id="${product.id}">
            ...
            <h3>${productName}</h3>
            ${productSubtitle ? `<p class="product-card__subtitle">${productSubtitle}</p>` : ''}
            ${highlightsHTML}
...
async function handleCategoryFilter(event) {
    ...
    try {
        ...
        const fallbackProducts = categoryId === 'all'
            ? localProductsData
            : localProductsData.filter(product => String(product.category_id) === String(categoryId));
...
const localCategoriesData = [
    { id: 1, name: 'Sensors', name_zh: '传感器' },
    { id: 2, name: 'Controllers', name_zh: '控制器' }
];
...
const specificationLabels = {
    ...
    accuracy: { en: 'Accuracy', zh: '精度' },
    response_time: { en: 'Response Time', zh: '响应时间' },
    output: { en: 'Output', zh: '输出' },
    lifetime: { en: 'Lifetime', zh: '寿命' }
};
...
function renderProductModal(product) {
    ...
    const productSubtitle = currentLanguage === 'zh' ? product.subtitle_zh : product.subtitle;
    ...
    const modalDescription = [productSubtitle, productDescription].filter(Boolean).join(' ');
            ...
            <div id="modal-product-content">
                <div class="product-header">
                    ...
                    <div class="product-info">
                        ...
                        <p class="product-description">${modalDescription}</p>
                    </div>
                </div>
                ...
                <div class="product-specifications">
                    ...
                    <div class="specifications-grid">
                        ${Object.entries(product.specifications).map(([key, value]) => {
                            ...
                            const wideKeys = ['certification', 'certifications', 'standard', 'standards', 'compatibility', 'applications'];
                            const spanClass = wideKeys.includes(key.toLowerCase()) ? ' spec-item--wide' : '';
                            return `
                                <div class="spec-item${spanClass}">
